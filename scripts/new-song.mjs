...
function buildStreamingLinks({ spotify, appleMusic, youtube, bandcamp, other }) {
    const links = [];
...
function renderYamlLinks(items, indent = "  ") {
    if (!items.length) return `${indent}[]`;
    return items
        .map((x) => `${indent}- label: ${yamlEscape(x.label)}\n${indent}  url: ${yamlEscape(x.url)}`)
        .join("\n");
}
...
const res = await prompts(
    [
        ...
        {
            name: "otherLinks",
            message: 'Other links (optional, CSV of Label=URL, e.g. "SoundCloud=https://..., Site=https://...")',
            type: "text",
        },
        ...
        // If links are used: ask for URLs
        {
            ...
            type: (_, values) => (values.audioMode === "links" || values.audioMode === "both" ? "text" : null),
        },
        ...
        {
            name: "imageFiles",
            message: 'Image filenames (CSV) (e.g. "cover.jpg" or "cover.jpg, alt.png")',
            type: (_, values) => (values.hasImage ? "text" : null),
            initial: "cover.jpg",
        },
        {
            name: "copyExampleImage",
            message: "Copy example image into the first image filename (if available)?",
...
if ((audioMode === "files" || audioMode === "both") && parsedAudioFiles.length) {
    ...
    try {
        ...
        try {
            ...
            warnings.push(`Could not copy example audio: ${err?.message || String(err)}`);
        }
    }
}
...
if (hasImage && copyExampleImage && parsedImageFiles.length) {
    ...
    try {
        ...
        try {
            ...
            warnings.push(`Could not copy example image: ${err?.message || String(err)}`);
        }
    }
}
